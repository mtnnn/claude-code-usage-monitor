...
// Captive portal overlay (setup mode), sopra l'UI principale. Nasconde i 4
// pannelli tramite gli helper del core (ui_hide_main_panels /
// ui_restore_active_panel), che possiedono panels[]/active_panel.
...
void UsageUI_ShowPortal(const char* ap_name, const char* ap_ip, const char* ap_pass) {
  ...
  // Nascondi i 4 panel principali (la status bar resta sotto, coperta dal
  // portal_root).
  ui_hide_main_panels();
}
